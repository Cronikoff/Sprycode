"""Phase 173: Report-level stateGainAttributionResidualCoverageAbsolute metric.

stateGainAttributionResidualCoverageAbsolute =
    abs(stateGainAttributionResidualCoverage)
(when residual coverage is numeric)
"""
...
class TestPathwayStateGainAttributionResidualCoverageAbsolute:
    def test_single_target_matches_formula(self):
        ...
        if residual_coverage is None:
            assert rep["stateGainAttributionResidualCoverageAbsolute"] is None
        else:
            expected = abs(residual_coverage)
            assert abs(rep["stateGainAttributionResidualCoverageAbsolute"] - expected) < 1e-9
