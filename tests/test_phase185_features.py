"""Phase 185: Report-level stateGainAttributionCoverageAbsoluteResidualCoverage metric.

stateGainAttributionCoverageAbsoluteResidualCoverage =
    stateGainAttributionCoverageAbsoluteResidual / stateGainAttributionCoverageSum
...
class TestPathwayStateGainAttributionCoverageAbsoluteResidualCoverage:
    def test_single_target_matches_formula(self):
        ...
        """)
        rep = val(i, "rep")
        absolute_residual = rep["stateGainAttributionCoverageAbsoluteResidual"]
        coverage_sum = rep["stateGainAttributionCoverageSum"]
        if absolute_residual is None or coverage_sum is None or coverage_sum == 0:
            assert rep["stateGainAttributionCoverageAbsoluteResidualCoverage"] is None
        else:
            expected = absolute_residual / coverage_sum
            assert abs(rep["stateGainAttributionCoverageAbsoluteResidualCoverage"] - expected) < 1e-9
