"""Phase 275: Report-level stateGainAttributionCoverageResidual metric."""
...
class TestPhase275:
    def test_single_target_matches_formula(self):
        ...
        s = rep["stateGainAttributionCoverageSum"]
        if s is None:
            assert rep["stateGainAttributionCoverageResidual"] is None
        else:
            assert abs(rep["stateGainAttributionCoverageResidual"] - (1.0 - s)) < 1e-9
