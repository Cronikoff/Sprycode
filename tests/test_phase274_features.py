"""Phase 274: Report-level stateGainAttributionCoverageSum metric."""
...
class TestPhase274:
    def test_single_target_matches_formula(self):
        ...
        a = rep["reportStateGainCoverage"]
        b = rep["preTargetStateGainCoverage"]
        c = rep["stateGainAttributionResidualCoverage"]
        if a is None or b is None or c is None:
            assert rep["stateGainAttributionCoverageSum"] is None
        else:
            assert abs(rep["stateGainAttributionCoverageSum"] - (a + b + c)) < 1e-9
