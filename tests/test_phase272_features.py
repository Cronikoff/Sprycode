"""Phase 272: Report-level stateGainAttributionResidualPerTarget metric."""
...
class TestPhase272:
    def test_single_target_matches_formula(self):
        ...
        base = rep["stateGainAttributionResidual"]
        count = len(rep["targets"])
        if base is None or count == 0:
            assert rep["stateGainAttributionResidualPerTarget"] is None
        else:
            assert abs(rep["stateGainAttributionResidualPerTarget"] - (base / count)) < 1e-9
