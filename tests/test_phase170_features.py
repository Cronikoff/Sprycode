"""Phase 170: Report-level stateGainAttributionResidualCoveragePerCycle metric.

stateGainAttributionResidualCoveragePerCycle =
    stateGainAttributionResidualCoverage / cycles
(when cycles > 0 and the residual coverage is numeric)
"""
...
class TestPathwayStateGainAttributionResidualCoveragePerCycle:
    def test_single_target_matches_formula(self):
        ...
        cycles = rep["cycles"]
        if residual_coverage is None or cycles == 0:
            assert rep["stateGainAttributionResidualCoveragePerCycle"] is None
        else:
            expected = residual_coverage / cycles
            assert abs(rep["stateGainAttributionResidualCoveragePerCycle"] - expected) < 1e-9
