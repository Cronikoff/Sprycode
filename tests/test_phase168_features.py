"""Phase 168: Report-level stateGainAttributionCoverageSumPerAttempt metric.

stateGainAttributionCoverageSumPerAttempt =
    stateGainAttributionCoverageSum / totalAttempts
(when totalAttempts > 0 and the coverage sum is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageSumPerAttempt:
    def test_single_target_matches_formula(self):
        ...
        """)
        rep = val(i, "rep")
        cov_sum = rep["stateGainAttributionCoverageSum"]
        total_attempts = rep["totalAttempts"]
        if cov_sum is None or total_attempts == 0:
            assert rep["stateGainAttributionCoverageSumPerAttempt"] is None
        else:
            expected = cov_sum / total_attempts
            assert abs(rep["stateGainAttributionCoverageSumPerAttempt"] - expected) < 1e-9
