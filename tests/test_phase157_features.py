"""Phase 157: Report-level total-attempts-per-cycle metric.

totalAttemptsPerCycle = totalAttempts / cycles (when cycles > 0)
"""
...
class TestPathwayReportTotalAttemptsPerCycle:
    def test_single_target_matches_formula(self):
        ...
        """)
        rep = val(i, "rep")
        expected = rep["totalAttempts"] / rep["cycles"]
        assert abs(rep["totalAttemptsPerCycle"] - expected) < 1e-9
    ...
    def test_no_managed_steps_has_none(self):
        ...
        assert rep["totalAttemptsPerCycle"] is None
    ...
    def test_value_is_positive_when_attempts_exist(self):
        i = run("""
            let orch = Orchestrator.new()
            orch.addManagedStep(
                "svc",
        ...
        rep = val(i, "rep")
        assert rep["totalAttempts"] > 0
        assert rep["totalAttemptsPerCycle"] > 0
