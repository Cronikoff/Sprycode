"""Phase 203: Report-level stateGainAttributionCoverageResidualCoverageAbsolutePerAttempt metric.

stateGainAttributionCoverageResidualCoverageAbsolutePerAttempt =
    stateGainAttributionCoverageResidualCoverageAbsolute / totalAttempts
(when denominator > 0 and absolute metric is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageResidualCoverageAbsolutePerAttempt:
    def test_single_target_matches_formula(self):
        ...
        if absolute is None or attempts == 0:
            assert rep["stateGainAttributionCoverageResidualCoverageAbsolutePerAttempt"] is None
        else:
            expected = absolute / attempts
            assert abs(rep["stateGainAttributionCoverageResidualCoverageAbsolutePerAttempt"] - expected) < 1e-9
