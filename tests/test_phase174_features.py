"""Phase 174: Report-level stateGainAttributionResidualCoverageAbsolutePerCycle metric.

stateGainAttributionResidualCoverageAbsolutePerCycle =
    stateGainAttributionResidualCoverageAbsolute / cycles
(when cycle count > 0 and the absolute residual coverage is numeric)
"""
...
class TestPathwayStateGainAttributionResidualCoverageAbsolutePerCycle:
    def test_single_target_matches_formula(self):
        ...
        if absolute is None or cycles == 0:
            assert rep["stateGainAttributionResidualCoverageAbsolutePerCycle"] is None
        else:
            expected = absolute / cycles
            assert abs(rep["stateGainAttributionResidualCoverageAbsolutePerCycle"] - expected) < 1e-9
