"""Phase 160: Report-level pre-target-state-gain-per-target metric.

preTargetStateGainPerTarget = preTargetStateGain / len(targets) (when target
count > 0 and report state gain is numeric)
"""
...
class TestPathwayReportPreTargetStateGainPerTarget:
    def test_single_target_matches_formula(self):
        ...
        """)
        rep = val(i, "rep")
        expected = rep["preTargetStateGain"] / len(rep["targets"])
        assert abs(rep["preTargetStateGainPerTarget"] - expected) < 1e-9
    ...
    def test_no_managed_steps_has_none(self):
        ...
        rep = val(i, "rep")
        assert rep["targets"] == []
        assert rep["preTargetStateGainPerTarget"] is None
