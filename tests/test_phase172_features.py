"""Phase 172: Report-level stateGainAttributionResidualCoveragePerTarget metric.

stateGainAttributionResidualCoveragePerTarget =
    stateGainAttributionResidualCoverage / len(targets)
(when target count > 0 and the residual coverage is numeric)
"""
...
class TestPathwayStateGainAttributionResidualCoveragePerTarget:
    def test_single_target_matches_formula(self):
        ...
        """)
        rep = val(i, "rep")
        residual_coverage = rep["stateGainAttributionResidualCoverage"]
        target_count = len(rep["targets"])
        if residual_coverage is None or target_count == 0:
            assert rep["stateGainAttributionResidualCoveragePerTarget"] is None
        else:
            expected = residual_coverage / target_count
            assert abs(rep["stateGainAttributionResidualCoveragePerTarget"] - expected) < 1e-9
