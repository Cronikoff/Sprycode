"""Phase 190: Report-level stateGainAttributionCoverageResidualCoveragePerCycle metric.

stateGainAttributionCoverageResidualCoveragePerCycle =
    stateGainAttributionCoverageResidualCoverage / cycles
(when cycle count > 0 and residual coverage metric is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageResidualCoveragePerCycle:
    def test_single_target_matches_formula(self):
        ...
        if coverage is None or cycles == 0:
            assert rep["stateGainAttributionCoverageResidualCoveragePerCycle"] is None
        else:
            expected = coverage / cycles
            assert abs(rep["stateGainAttributionCoverageResidualCoveragePerCycle"] - expected) < 1e-9
