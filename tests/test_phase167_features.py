"""Phase 167: Report-level stateGainAttributionCoverageSumPerCycle metric.

stateGainAttributionCoverageSumPerCycle =
    stateGainAttributionCoverageSum / cycles
(when cycles > 0 and the coverage sum is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageSumPerCycle:
    def test_single_target_matches_formula(self):
        ...
        cycles = rep["cycles"]
        if cov_sum is None or cycles == 0:
            assert rep["stateGainAttributionCoverageSumPerCycle"] is None
        else:
            expected = cov_sum / cycles
            assert abs(rep["stateGainAttributionCoverageSumPerCycle"] - expected) < 1e-9
