"""Phase 204: Report-level stateGainAttributionCoverageResidualCoverageAbsolutePerTarget metric.

stateGainAttributionCoverageResidualCoverageAbsolutePerTarget =
    stateGainAttributionCoverageResidualCoverageAbsolute / targets.length
...
class TestPathwayStateGainAttributionCoverageResidualCoverageAbsolutePerTarget:
    def test_single_target_matches_formula(self):
        ...
        if absolute is None or count == 0:
            assert rep["stateGainAttributionCoverageResidualCoverageAbsolutePerTarget"] is None
        else:
            expected = absolute / count
            assert abs(rep["stateGainAttributionCoverageResidualCoverageAbsolutePerTarget"] - expected) < 1e-9
