"""Phase 164: Report-level stateGainAttributionCoverageResidualPerCycle metric.

stateGainAttributionCoverageResidualPerCycle =
    stateGainAttributionCoverageResidual / cycles
(when cycle count > 0 and the coverage residual is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageResidualPerCycle:
    def test_single_target_matches_formula(self):
        ...
        cycles = rep["cycles"]
        if residual is None or cycles == 0:
            assert rep["stateGainAttributionCoverageResidualPerCycle"] is None
        else:
            expected = residual / cycles
            assert abs(rep["stateGainAttributionCoverageResidualPerCycle"] - expected) < 1e-9
