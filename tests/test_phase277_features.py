"""Phase 277: Report-level stateGainAttributionCoverageAbsoluteResidualPerCycle metric."""
...
class TestPhase277:
    def test_single_target_matches_formula(self):
        ...
        base = rep["stateGainAttributionCoverageAbsoluteResidual"]
        cycles = rep["cycles"]
        if base is None or cycles == 0:
            assert rep["stateGainAttributionCoverageAbsoluteResidualPerCycle"] is None
        else:
            assert abs(rep["stateGainAttributionCoverageAbsoluteResidualPerCycle"] - (base / cycles)) < 1e-9
