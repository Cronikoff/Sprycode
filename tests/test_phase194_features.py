"""Phase 194: Report-level stateGainAttributionCoverageResidualAbsolutePerCycle metric.

stateGainAttributionCoverageResidualAbsolutePerCycle =
    stateGainAttributionCoverageResidualAbsolute / cycles
(when cycles > 0 and absolute metric is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageResidualAbsolutePerCycle:
    def test_single_target_matches_formula(self):
        ...
        if absolute is None or cycles == 0:
            assert rep["stateGainAttributionCoverageResidualAbsolutePerCycle"] is None
        else:
            expected = absolute / cycles
            assert abs(rep["stateGainAttributionCoverageResidualAbsolutePerCycle"] - expected) < 1e-9
