"""Phase 195: Report-level stateGainAttributionCoverageResidualAbsolutePerAttempt metric.

stateGainAttributionCoverageResidualAbsolutePerAttempt =
    stateGainAttributionCoverageResidualAbsolute / totalAttempts
(when totalAttempts > 0 and absolute metric is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageResidualAbsolutePerAttempt:
    def test_single_target_matches_formula(self):
        ...
        if absolute is None or attempts == 0:
            assert rep["stateGainAttributionCoverageResidualAbsolutePerAttempt"] is None
        else:
            expected = absolute / attempts
            assert abs(rep["stateGainAttributionCoverageResidualAbsolutePerAttempt"] - expected) < 1e-9
