"""Phase 165: Report-level stateGainAttributionCoverageResidualPerAttempt metric.

stateGainAttributionCoverageResidualPerAttempt =
    stateGainAttributionCoverageResidual / totalAttempts
(when totalAttempts > 0 and the coverage residual is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageResidualPerAttempt:
    def test_single_target_matches_formula(self):
        ...
        total_attempts = rep["totalAttempts"]
        if residual is None or total_attempts == 0:
            assert rep["stateGainAttributionCoverageResidualPerAttempt"] is None
        else:
            expected = residual / total_attempts
            assert abs(rep["stateGainAttributionCoverageResidualPerAttempt"] - expected) < 1e-9
