"""Phase 210: Report-level stateGainAttributionResidualCoverageAbsoluteCoveragePerCycle metric."""
...
class TestPathwayStateGainAttributionResidualCoverageAbsoluteCoveragePerCycle:
    def test_single_target_matches_formula(self):
        ...
        if coverage is None or cycles == 0:
            assert rep["stateGainAttributionResidualCoverageAbsoluteCoveragePerCycle"] is None
        else:
            expected = coverage / cycles
            assert abs(rep["stateGainAttributionResidualCoverageAbsoluteCoveragePerCycle"] - expected) < 1e-9
