"""Phase 166: Report-level stateGainAttributionCoverageResidualPerTarget metric.

stateGainAttributionCoverageResidualPerTarget =
    stateGainAttributionCoverageResidual / targetCount
(when targetCount > 0 and the coverage residual is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageResidualPerTarget:
    def test_single_target_matches_formula(self):
        ...
        if residual is None or target_count == 0:
            assert rep["stateGainAttributionCoverageResidualPerTarget"] is None
        else:
            expected = residual / target_count
            assert abs(rep["stateGainAttributionCoverageResidualPerTarget"] - expected) < 1e-9
