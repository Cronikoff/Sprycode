"""Phase 200: Report-level stateGainAttributionCoverageResidualAbsoluteCoveragePerTarget metric.

stateGainAttributionCoverageResidualAbsoluteCoveragePerTarget =
    stateGainAttributionCoverageResidualAbsoluteCoverage / targets.length
(when targets.length > 0 and residual absolute coverage metric is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageResidualAbsoluteCoveragePerTarget:
    def test_single_target_matches_formula(self):
        ...
        if coverage is None or count == 0:
            assert rep["stateGainAttributionCoverageResidualAbsoluteCoveragePerTarget"] is None
        else:
            expected = coverage / count
            assert abs(rep["stateGainAttributionCoverageResidualAbsoluteCoveragePerTarget"] - expected) < 1e-9
