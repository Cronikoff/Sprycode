"""Phase 147: Report-level state-gain attribution residual metric.

stateGainAttributionResidual =
    reportStateGain - (targetStateGainSum + preTargetStateGain)
"""
...
class TestPathwayReportStateGainAttributionResidual:
    def test_single_target_has_zero_residual(self):
        i = run("""
        ...
        """)
        rep = val(i, "rep")
        assert abs(rep["stateGainAttributionResidual"]) < 1e-9

    def test_two_target_has_zero_residual(self):
        i = run("""
            let orch = Orchestrator.new()
            orch.addManagedStep(
                "svcA",
    ...
    def test_residual_matches_formula(self):
        i = run("""
            let orch = Orchestrator.new()
            orch.addManagedStep(
                "svcA",
    ...
    def test_zero_report_gain_with_managed_target_has_zero_residual(self):
        i = run("""
            let orch = Orchestrator.new()
            orch.addManagedStep(
                "svc",
