"""Phase 212: Report-level stateGainAttributionResidualCoverageAbsoluteCoveragePerTarget metric."""
...
class TestPathwayStateGainAttributionResidualCoverageAbsoluteCoveragePerTarget:
    def test_single_target_matches_formula(self):
        ...
        if coverage is None or count == 0:
            assert rep["stateGainAttributionResidualCoverageAbsoluteCoveragePerTarget"] is None
        else:
            expected = coverage / count
            assert abs(rep["stateGainAttributionResidualCoverageAbsoluteCoveragePerTarget"] - expected) < 1e-9
