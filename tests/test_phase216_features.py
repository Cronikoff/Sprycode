"""Phase 216: Report-level stateGainAttributionResidualAbsoluteCoverageAbsolutePerTarget metric."""
...
class TestPathwayStateGainAttributionResidualAbsoluteCoverageAbsolutePerTarget:
    def test_single_target_matches_formula(self):
        ...
        if absolute is None or count == 0:
            assert rep["stateGainAttributionResidualAbsoluteCoverageAbsolutePerTarget"] is None
        else:
            expected = absolute / count
            assert abs(rep["stateGainAttributionResidualAbsoluteCoverageAbsolutePerTarget"] - expected) < 1e-9
