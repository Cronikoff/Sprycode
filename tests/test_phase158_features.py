"""Phase 158: Report-level total-attempts-per-target metric.

totalAttemptsPerTarget = totalAttempts / len(targets) (when target count > 0)
"""
...
class TestPathwayReportTotalAttemptsPerTarget:
    def test_single_target_matches_formula(self):
        ...
        """)
        rep = val(i, "rep")
        expected = rep["totalAttempts"] / len(rep["targets"])
        assert abs(rep["totalAttemptsPerTarget"] - expected) < 1e-9
    ...
    def test_no_managed_steps_has_none(self):
        ...
        assert rep["totalAttemptsPerTarget"] is None
    ...
    def test_value_is_positive_when_attempts_exist(self):
        ...
        rep = val(i, "rep")
        assert rep["totalAttempts"] > 0
        assert rep["totalAttemptsPerTarget"] > 0
