"""Phase 169: Report-level stateGainAttributionCoverageSumPerTarget metric.

stateGainAttributionCoverageSumPerTarget =
    stateGainAttributionCoverageSum / len(targets)
(when target count > 0 and the coverage sum is numeric)
"""
...
class TestPathwayStateGainAttributionCoverageSumPerTarget:
    def test_single_target_matches_formula(self):
        ...
        if cov_sum is None or target_count == 0:
            assert rep["stateGainAttributionCoverageSumPerTarget"] is None
        else:
            expected = cov_sum / target_count
            assert abs(rep["stateGainAttributionCoverageSumPerTarget"] - expected) < 1e-9
